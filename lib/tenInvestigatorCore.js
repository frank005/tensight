...
function parseTar(buffer) {
  ...
  while (offset < buf.length - 512) {
    ...
    const mtimeOctal = header.slice(136, 148).toString('utf8').replace(/\0+$/, '').trim();
    const mtime = parseInt(mtimeOctal, 8) || 0;

    offset += 512;

    // typeFlag '0' or '\0' = regular file; we only care about those.
    if (size > 0 && name && (typeFlag === '0' || typeFlag === '\0')) {
      entries.push({ name, data: buf.slice(offset, offset + size), size, typeFlag, mtime });
    }
...
function pickErrEntry(entries) {
  ...
  const exactTenErr = nonEmpty.find((e) => entryBaseName(e).toLowerCase() === 'ten.err');
  if (exactTenErr) return exactTenErr;

  const tenVariantErrs = nonEmpty
    .filter((e) => isMainErrName(entryBaseName(e)))
    .sort((a, b) => (b.mtime || 0) - (a.mtime || 0) || b.data.length - a.data.length);
  if (tenVariantErrs.length) return tenVariantErrs[0];

  const logLike = nonEmpty
    .filter((e) => isLogLikeName(entryBaseName(e)))
    .sort((a, b) => (b.mtime || 0) - (a.mtime || 0) || b.data.length - a.data.length);
  if (logLike.length) return logLike[0];

  const sorted = nonEmpty.slice().sort((a, b) => (b.mtime || 0) - (a.mtime || 0) || b.data.length - a.data.length);
