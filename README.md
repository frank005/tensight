...
| **Turns (user & agent)** | Consolidated turn timeline; inline **`/think` badges** on the nearest transcript row when an external `/think` API call was processed during that turn (action + agent state + timing offset; full payload in Keypoints) |
